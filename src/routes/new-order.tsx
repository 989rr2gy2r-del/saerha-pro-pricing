...
function buildAliasSet(
  ...
  const aliasTerms = aliases
    ...
    .map((entry) => normalizeForMatch(entry.alias || entry.normalized_alias || ""))
    .filter(Boolean);

  return new Set(aliasTerms);
}
...
function findBestProductMatch(
  ...
  const raw = normalizeForMatch(itemText || "");
  if (!raw) {
    return {
      product: null,
      status: "UNMATCHED" as const,
      score: 0,
      reason: "لا يوجد نص واضح للبحث",
    };
  }

  let best: { product: ProductRecord | null; status: MatchStatus; score: number; reason: string } =
    {
      product: null,
      status: "UNMATCHED",
      score: 0,
      reason: "لا توجد مطابقة كافية",
    };
  ...
  for (const product of products) {
    ...
    const productTokens = new Set(tokenize(combined));
    const overlap = [...rawTokens].filter((token) => productTokens.has(token)).length;
    const overlapRatio = productTokens.size ? overlap / Math.max(productTokens.size, 1) : 0;

    const skuMatch = product.sku ? raw.includes(normalizeForMatch(product.sku)) : false;
    const nameMatch =
      normalizeForMatch(product.name_ar || "").length > 0
        ? raw.includes(normalizeForMatch(product.name_ar || ""))
        : false;
    const englishMatch = product.name_en ? raw.includes(normalizeForMatch(product.name_en)) : false;
    const shortMatch = product.short_name
      ? raw.includes(normalizeForMatch(product.short_name))
      : false;
    const brandMatch = product.brand ? raw.includes(normalizeForMatch(product.brand)) : false;
    const modelMatch = product.model ? raw.includes(normalizeForMatch(product.model)) : false;
    const sizeMatch = product.size ? raw.includes(normalizeForMatch(product.size)) : false;
    const aliasMatches = buildAliasSet(product, aliases);
    const aliasExact = [...aliasMatches].some((alias) => raw.includes(alias));

    let score = 0;
    if (skuMatch) score += 0.9;
    if (nameMatch) score += 0.75;
    if (englishMatch) score += 0.7;
    if (shortMatch) score += 0.55;
    if (brandMatch) score += 0.2;
    if (modelMatch) score += 0.2;
    if (sizeMatch) score += 0.15;
    if (aliasExact) score += 0.6;
    score += Math.min(0.55, overlapRatio * 0.35);

    if (score > 0 && score > best.score) {
      let reason = "لا توجد مطابقة كافية";
      if (skuMatch) reason = "تطابق مباشر مع SKU";
      else if (nameMatch || englishMatch || shortMatch)
        reason = "مطابقة اسم المنتج مع البيانات المتاحة";
      else if (aliasExact) reason = "مطابقة عبر اسم مستعار محفوظ";
      else if (brandMatch || modelMatch || sizeMatch)
        reason = "تشابه جزئي في الماركة/الموديل/المقاس يحتاج مراجعة";
      else if (overlapRatio > 0.15) reason = "تشابه نصي منخفض يحتاج مراجعة يدوية";

      const status: MatchStatus =
        score >= 0.78 ? "HIGH_CONFIDENCE" : score >= 0.45 ? "NEEDS_REVIEW" : "UNMATCHED";
      best = { product: score >= 0.45 ? product : null, status, score, reason };
    }
  }

  if (best.product && best.status === "UNMATCHED") {
    best.product = null;
  }
